$(document).ready(function () {
  ...
  const perDurationKey = `quizPerDuration_${packetId}`;
  ...
  (function detectNoTimer() {
    const developerNoTimerPackets = [8];
  ...
  const perQuestionPacketSet = new Set([7, 9, 3, 2, 1, 6, 4, 5, 10]);
  ...
  const perQuestionIntervalDelay = 200;
  ...
  function tampilkanSoal(data, nomor) {
    ...
    const opsiHtml = (data.options || []).map(opt => {
      ...
      const isChecked = Array.isArray(stored) ? stored.includes(opt.value) : stored === opt.value;
